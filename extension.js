...
function activate(context) {
  ...
  const createComponentCommand = vscode.commands.registerCommand(
    'reactComponentGenerator.createComponent',
    async (uri) => {
      ...
      if (createStyle) {
        fs.writeFileSync(
          path.join(targetDir, styleFileName),
          `/* Styles for ${pascalComponentName} */\n`
        );
      }
  ...
  const openTemplatesCommand = vscode.commands.registerCommand(
    'reactComponentGenerator.openTemplateFolder',
    async () => {
      const templatesPath = getTemplatesPath();
